...
// Register shortcode
add_shortcode( 'politeia_add_book', function() {
    ...
    <div class="prs-add-book">
        ...
        <form id="prs-add-book-form" class="prs-form" method="post" enctype="multipart/form-data" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" style="display:none;">
            ...
            <label><?php _e('Year','politeia-reading'); ?>
                <input type="number" name="prs_year" min="1400" max="<?php echo esc_attr( date('Y') + 1 ); ?>" />
            </label>
...
function prs_add_book_submit_handler() {
    ...
    if ( ! isset( $_POST['prs_nonce'] ) || ! wp_verify_nonce( $_POST['prs_nonce'], 'prs_add_book' ) ) wp_die( 'Invalid nonce.' );

    $user_id = get_current_user_id();

    $title  = isset($_POST['prs_title'])  ? sanitize_text_field($_POST['prs_title'])  : '';
    $author = isset($_POST['prs_author']) ? sanitize_text_field($_POST['prs_author']) : '';
    $year   = isset($_POST['prs_year']) && $_POST['prs_year'] !== '' ? absint($_POST['prs_year']) : null;
    ...
    prs_ensure_user_book( $user_id, $book_id );
