...
class Politeia_Reading_REST {
    ...
    public static function register() {
        register_rest_route('politeia/v1', '/user-books/(?P<id>\d+)', [
            'methods'  => 'POST',
            'callback' => [__CLASS__, 'update_user_book'],
    ...
    public static function update_user_book( WP_REST_Request $req ) {
        ...
        $valid_reading = ['not_started','started','finished'];
        $valid_owning  = ['in_shelf','lost','borrowed','borrowing','sold'];
        if ( $reading && ! in_array($reading, $valid_reading, true) ) $reading = '';
        if ( $owning  && ! in_array($owning,  $valid_owning,  true) ) $owning  = '';
        ...
        if ( (int)$owner !== (int)$user_id ) return new WP_REST_Response(['error'=>'forbidden'], 403);

        $data = [];
        if ($reading) $data['reading_status'] = $reading;
        if ($owning)  $data['owning_status']  = $owning;
        if (!$data) return new WP_REST_Response(['ok'=>true], 200);
