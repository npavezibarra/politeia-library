...
class Politeia_Post_Reading_REST {
	...
	public static function register_routes() {
		// POST /toggle
		register_rest_route( self::NS, '/' . self::BASE . '/toggle', [
		...
		// POST /start
		register_rest_route( self::NS, '/' . self::BASE . '/start', [
			'methods'             => WP_REST_Server::CREATABLE,
		...
		// POST /finish
		register_rest_route( self::NS, '/' . self::BASE . '/finish', [
			'methods'             => WP_REST_Server::CREATABLE,
			'callback'            => [ __CLASS__, 'handle_finish' ],
			'permission_callback' => [ __CLASS__, 'require_logged_in' ],
			'args'                => self::post_args_schema(),
		] );
		...
		register_rest_route( self::NS, '/' . self::BASE . '/status', [
			...
			'args'                => [
				'post_id' => [
					...
					'validate_callback' => [ __CLASS__, 'validate_post_exists' ],
	...
	public static function require_logged_in( $request ) {
		return is_user_logged_in() ? true : new WP_Error( 'not_logged_in', 'Authentication required.', [ 'status' => 401 ] );
	}
	...
	protected static function error_response( $error ) {
		if ( is_wp_error( $error ) ) {
			...
			return self::respond( [ 'error' => $code, 'message' => $msg ], $status );
		}
		return self::respond( [ 'error' => 'unknown_error', 'message' => 'Unknown error' ], 500 );
	}
	...
	public static function handle_toggle( WP_REST_Request $request ) {
		...
		if ( ! class_exists( 'Politeia_Post_Reading_Manager' ) ) {
			return self::respond( [ 'error' => 'missing_manager', 'message' => 'Manager class not loaded.' ], 500 );
		}
...
add_action( 'init', [ 'Politeia_Post_Reading_REST', 'init' ] );
